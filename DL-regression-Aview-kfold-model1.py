...
project_path = 'E:/'
df = pd.read_excel(f'{project_path}/I3D-features.xlsx')
trial_name = "500I3D-CNN-model"

df_normalized = df
train_data = df_normalized[df_normalized['set_split'] == 'train']
valid_data = df_normalized[df_normalized['set_split'] == 'valid']
test_data = df_normalized[df_normalized['set_split'] == 'test']

# 分离特征和标签
X_train = train_data.iloc[:, 8:].values # 8; 4
y_train = train_data.iloc[:, 3].values
X_valid = valid_data.iloc[:, 8:].values # 8; 4
y_valid = valid_data.iloc[:, 3].values
X_test = test_data.iloc[:, 8:].values # 8; 4
...
def create_model():
    ...
    output = Dense(1, activation='linear')(input1)
