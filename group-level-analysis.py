...
num_y = 3 # FEV1-3, FVC-3

idx_A = list(range(4, 8)) # ASHW, FEV1-4:8, FVC-4:8
idx_B = list(range(8, 25)) # LAA, FEV1-8:25, FVC-8:23
idx_C = list(range(25, 51)) # VESSEL, FEV1-25:51, FVC-23:52
idx_D = list(range(51, 102)) # AIRWAY, FEV1-51:102, FVC-52:145

# 合并特征索引
trial_name = "ABDC-lasso-Aview-CNN-model"
idx_group = idx_A + idx_B + idx_D + idx_C
# 选出特征
X_train = train_data.iloc[:, idx_group].values
X_valid = valid_data.iloc[:, idx_group].values
X_test = test_data.iloc[:, idx_group].values

y_train = train_data.iloc[:, num_y].values
y_valid = valid_data.iloc[:, num_y].values
y_test = test_data.iloc[:, num_y].values
